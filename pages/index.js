    ...
    <div className="flex flex-col items-center justify-center min-h-screen p-4 font-sans" style={{ fontFamily: 'Poppins, sans-serif' }}>
      {answers.length < questions.length ? (
        ...
        <div className="w-full max-w-md text-center">
          ...
          <button className="mt-6 px-6 py-3 bg-gradient-to-r from-blue-400 to-blue-600 text-white rounded-full shadow-md transform transition-transform hover:scale-105 active:scale-95" onClick={() => { setStep(0); setAnswers([]); setTotalScore(0); }}>もう一度診断する</button>
        </div>
      )}
    </div>
  );
